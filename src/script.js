...
mesh2.position.x = - 2
mesh3.position.x = 2
mesh1.scale.multiplyScalar(0.8);
mesh2.scale.multiplyScalar(0.8);
mesh3.scale.multiplyScalar(0.8);
...
for(let i = 0; i < particlesCount; i++){
    ...
    // start high enough and then spread far enough below so that we reach the end with the scroll
...
window.addEventListener('mousemove', (event) =>{
    cursor.x = event.clientX / sizes.width - 0.5    // clientX is getting bigger going right
    cursor.y = -(event.clientY / sizes.height - 0.5) // clientY is getting bigger going downwards
